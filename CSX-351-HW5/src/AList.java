...
class AList<T> implements ListInterface<T> {
    ...
    */
    public void reverse()
    {
    	...
    	if(!this.isEmpty())
    	{
    		int x=this.getLength(); // The current length of the list.
    		int swap1=1,swap2=x;
    		T temp;
    		//Reversing by swapping ith element from beginning and jth element from end.
    		while(swap1<swap2)
    		{
    			temp=this.getEntry(swap1); 
    			this.replace(swap1,this.getEntry(swap2));
    			this.replace(swap2, temp);
    			swap1++;swap2--; // Increment swap1 and decrement swap2 for next iterations.
